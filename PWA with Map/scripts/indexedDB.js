...
var dbb;
var request = window.indexedDB.open("dbasevone", 1);
...
request.onsuccess = function(event) {
  dbb = request.result;
  console.log("success: "+ dbb);
};
 
request.onupgradeneeded = function(event) {
        var dbb = event.target.result;
        var objectStore = dbb.createObjectStore("cafe", {keyPath: "id"});
        for (var i in cafe) {
                objectStore.add(cafe[i]);      
        }
    
        var objectStore2 = dbb.createObjectStore("menu", {keyPath: "id"});
        for (var i in menu) {
...
var printCafe = function(){
    var iterate = 0;
    var j = 0;
    var objectStore = dbb.transaction("cafe").objectStore("cafe");
          ...
          if (cursor) {
              if(availability==undefined && ambience.length == 0){
                  ...
                  if(availability==undefined && ambience.length != 0){
                      ...
                      if(categories.length == 0 && ambience.length == 0){
                          console.log("You");
...
var printMenu = function(id){
    var ids ="a"+id+"b";
    var objectStore = dbb.transaction("menu").objectStore("menu");
